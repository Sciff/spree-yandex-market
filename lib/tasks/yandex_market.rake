...
namespace :spree_yandex_market do
  ...
  def generate_export_file torgovaya_sistema='yandex_market'
    begin
      puts "#{Time.now} start export...#{torgovaya_sistema}"
      ...
      directory = File.join(path, "#{torgovaya_sistema}")
      ...
      tfile.write(yml_xml)
      tfile.close
      puts "#{tfile.path} -> #{File.join(directory, "#{torgovaya_sistema}.xml")}"
      ...
      puts "#{Time.now} remove unnecessary files"
      ...
      @export_files[@number_of_files..-1] && @export_files[@number_of_files..-1].each do |x|
        if File.exist?(File.join(directory, x.first))
          ...
          puts "[ #{torgovaya_sistema} ] удаляем устаревший файл"
          Rails.logger.info "[ #{torgovaya_sistema} ] путь к файлу #{File.join(directory, x.first)}"
          puts "[ #{torgovaya_sistema} ] путь к файлу #{File.join(directory, x.first)}"
